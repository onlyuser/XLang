                ...
                node::TermInternalType<node::NodeIdentIFace::STRING>::type value)
{
    return new (PNEW(tc->alloc(), node::, NodeIdentIFace))
            node::TermNode<node::NodeIdentIFace::STRING>(sym_id, loc, value); // supports non-trivial dtor
}
...
static node::NodeIdentIFace* visit(TreeContext* tc, ticpp::Node* node)
{
    ...
    {
        node::SymbolNode* dest_node = mvc::MVCModel::make_symbol(tc, name_to_id(node_type), dummy_loc, 0);
        ticpp::Iterator<ticpp::Node> child;
        for(child = child.begin(node); child != child.end(); child++)
            dest_node->push_back(visit(tc, child.Get()));
        return dest_node;
