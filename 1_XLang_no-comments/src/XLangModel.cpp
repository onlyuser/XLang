...
node::NodeBase* Model::make_value(ParseContextBase* pc, uint32 sym_id, YYLTYPE &loc, float32 value)
{
    return new (pc->alloc(), __FILE__, __LINE__) node::ValueNode(sym_id, loc, value);
}

node::NodeBase* Model::make_ident(ParseContextBase* pc, uint32 sym_id, YYLTYPE &loc, const std::string* name)
{
    return new (pc->alloc(), __FILE__, __LINE__) node::IdentNode(sym_id, loc, pc->get_insert_name(*name));
}
